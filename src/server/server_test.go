	...
	testflight.WithServer(private, func(r *testflight.Requester) {
		...
		testCases := []struct {
			...
			},
			{
				...
				method:   "CUSTOM",
				body:     string([]byte(base64.StdEncoding.EncodeToString(createEncryptedTransactionForTest().Hash))),
				headers: http.Header{
		...
		for _, test := range testCases {
			...
			t.Run(test.name, func(t *testing.T) {
				...
				if test.followUpEndpoint == "/receiveraw" {

					encodedKey := response.Body
					log.Debug("Send Response: ", response)
					encodedTo := "OeVDzTdR95fhLKIgpBLxqdDNXYzgozgi7dnnS125A3w="
					header := http.Header{}
					header.Add(utils.HeaderKey, encodedKey)
					header.Add(utils.HeaderTo, encodedTo)
					reqRCV, _ := http.NewRequest("GET", test.followUpEndpoint, bytes.NewBufferString(""))
					reqRCV.Header = header
					followUpResponse = r.Do(reqRCV)
					require.Equal(t, test.body, followUpResponse.Body)
...
func createEncryptedTransactionForTest() *data.Encrypted_Transaction {
